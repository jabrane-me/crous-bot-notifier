...
def create_fallback_session() -> requests.Session:
    ...
    for cookie in cookies:
        if not isinstance(cookie, dict) or cookie.get("name") not in fallback_cookie_names:
            continue
    ...
    for target in notifier.load_targets():
        ...
        try:
            ...
            print(
                ...
                "retrying with a fresh PHPSESSID + tool.47.hasUserReadRules session."
            )
            fallback_session = create_fallback_session()
